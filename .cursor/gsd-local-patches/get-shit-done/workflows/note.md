...
- **Global scope**: `D:/programming/champ-app/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/programming/champ-app/.cursor/notes/`)
...
2. Glob `D:/programming/champ-app/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (D:/programming/champ-app/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/programming/champ-app/.cursor/notes/` — works in any directory
