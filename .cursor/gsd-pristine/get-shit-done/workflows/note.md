...
- **Global scope**: `D:/programming/champ/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/programming/champ/.cursor/notes/`)
...
2. Glob `D:/programming/champ/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (D:/programming/champ/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/programming/champ/.cursor/notes/` — works in any directory
