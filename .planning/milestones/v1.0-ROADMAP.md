...
Ship full fighter → service → slot → booking → payment → confirmation loop, plus My bookings/notifications and minimal admin.

## Phase Details (snapshot)

> Archived from `.planning/ROADMAP.md` at milestone close.
...
- Build on existing Ionic/Angular shell; add NestJS backend with Prisma/Postgres.
- Slots/booking state authoritative server-side with transactional concurrency guard.
- Stripe webhooks verify signature and are idempotent.

**Issues Deferred:**

- Human verification gaps for Phase 01 + Phase 03 (see `.planning/v1.0-MILESTONE-AUDIT.md` and `STATE.md` Deferred Items).
- Requirements traceability bookkeeping not updated for most v1 requirements (still shows Pending).
